...
def solve_lbvp_notSPD(L,f,B,g,N):
    ...
    if B.nnz == 0:
        u  = np.transpose([linalg.gmres(L, f)[0]])
    else:

        up = sp.csr_matrix.transpose(B) @ np.transpose([linalg.gmres((B @ sp.csr_matrix.transpose(B)),g)[0]])
        u0 = N @ np.transpose([linalg.gmres(sp.csr_matrix.transpose(N) @ L @ N,sp.csr_matrix.transpose(N) @ (f-L @ up))[0]])
