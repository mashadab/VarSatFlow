...
def adv_op(lmbda,q,Grid):
    ...
    A_w_pos = np.concatenate((np.zeros((1,Grid.N)),A_w_pos), axis=0)
    A_w_pos = np.concatenate((np.zeros((Grid.Nfx,1)),A_w_pos), axis=1)
    q_pos   = np.sign(np.maximum(q,0.0))
    
    A_w_neg = np.diagflat(lmbda)
    A_w_neg = np.concatenate((A_w_neg,np.zeros((1,Grid.N))), axis=0)
    A_w_neg = np.concatenate((A_w_neg,np.zeros((Grid.Nfx,1))), axis=1)   
    q_neg   =-np.sign(np.minimum(q,0.0))
